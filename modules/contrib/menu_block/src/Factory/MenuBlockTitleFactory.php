...
class MenuBlockTitleFactory implements MenuBlockTitleFactoryInterface {
   ...
   */
  public function __construct(EntityTypeManagerInterface $entityTypeManager, MenuActiveTrailInterface $menuActiveTrail, EntityRepositoryInterface $entityRepository) {
  ...
  private function getActiveTrailActiveItemTitle(MenuBlock $menuBlock) {
    try {
      return new MenuBlockActiveTrailActiveItemTitle($menuBlock, $this->menuActiveTrail, $this->entityRepository);
  ...
  private function getActiveTrailParentItemTitle(MenuBlock $menuBlock) {
    try {
      return new MenuBlockActiveTrailParentItemTitle($menuBlock, $this->menuActiveTrail, $this->entityRepository);
  ...
  private function getActiveTrailRootItemTitle(MenuBlock $menuBlock) {
    try {
      return new MenuBlockActiveTrailRootItemTitle($menuBlock, $this->menuActiveTrail, $this->entityRepository);
    } catch (Exception $exception) {
      return new MenuBlockEmptyTitle();
    }
  }
  ...
  private function getInitialMenuItemTitle($menuBlock) {
    try {
      return new MenuBlockInitialMenuItemTitle($menuBlock, $this->menuActiveTrail, $this->entityRepository);
    } catch (Exception $exception) {
      return new MenuBlockEmptyTitle();
    }
  }
